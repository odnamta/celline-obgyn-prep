...
/**
 * V8.3: Error fallback component for BulkImportPage
 * Displays when a child component crashes, keeping upload functionality accessible
 */
function BulkImportErrorFallback({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] p-8 text-center bg-slate-50 dark:bg-slate-800/30 rounded-lg border border-slate-200 dark:border-slate-700">
...
export default function BulkImportClient({ deckId }: BulkImportClientProps) {
  ...
  // V8.3: Page range state for precision scanning
  const [scanStartPage, setScanStartPage] = useState(1)
  const [scanEndPage, setScanEndPage] = useState(1)
  const [hasManuallyEditedStartPage, setHasManuallyEditedStartPage] = useState(false)
  ...
  // V8.3: Also initialize page range when PDF loads
  const handleDocumentReady = useCallback((pdf: PDFDocumentProxy) => {
    setPdfDocument(pdf)
    // V8.3: Initialize end page to total pages
    setScanEndPage(pdf.numPages)
  }, [])

  // V8.3: Handle page change in PDFViewer - sync start page unless manually edited
  const handlePdfPageChange = useCallback((pageNumber: number) => {
    if (!hasManuallyEditedStartPage) {
      setScanStartPage(pageNumber)
    }
  }, [hasManuallyEditedStartPage])
  ...
  // V8.3: Error boundary retry handler
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* V8.3: ErrorBoundary wraps main content for crash protection */}
      <ErrorBoundary fallback={<BulkImportErrorFallback onRetry={handleErrorRetry} />}>
              ...
              {pdfDocument && (
                <>
                  <AutoScanControls
                    ...
                    onStart={() => autoScan.startRangeScan({ startPage: scanStartPage, endPage: scanEndPage })}
                    ...
                    startPage={scanStartPage}
                    endPage={scanEndPage}
                    onStartPageChange={(page) => {
                      setScanStartPage(page)
                      setHasManuallyEditedStartPage(true)
                    }}
                    onEndPageChange={setScanEndPage}
                  />
                  <SkippedPagesPanel
